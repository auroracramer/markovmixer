...
def load_song(song_path):
    """
    Loads an individual song, normalizes it, and returns it. Essentia takes
    care of resampling if the song is not the required sampling rate.
    """
    data = ess.MonoLoader(filename=song_path, sampleRate=TARGET_FS)()
...
def load_songs(song_dir, max_songs=10, valid_exts=('.wav','.mp3')):
    """
